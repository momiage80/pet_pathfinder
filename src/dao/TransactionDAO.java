...
public class TransactionDAO {
	...
	public void insertTransaction(int userID, String type, int amount, int free, int paid, int receiver_id) {
		try {
			Class.forName("org.postgresql.Driver");
		}catch (Exception e) {
			// TODO: handle exception
		...
		try{
			...
			PreparedStatement pstmt = con.prepareStatement("insert into transactions (user_id, type, amount, afterfreecoin, afterpaidcoin, receiver_id) values (?,?,?,?,?,?);");
			pstmt.setInt(1, userID);
			pstmt.setString(2, type);
			pstmt.setInt(3, amount);
			pstmt.setInt(4, free);
			pstmt.setInt(5, paid);
			pstmt.setInt(6, receiver_id);
